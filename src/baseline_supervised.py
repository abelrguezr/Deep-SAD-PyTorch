...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model,
         ...
         lr, n_epochs, lr_milestone, batch_size, weight_decay, pretrain,
         ae_optimizer_name, ae_lr, ae_n_epochs, ae_lr_milestone, ae_batch_size,
         ae_weight_decay, num_threads, n_jobs_dataloader, normal_class,
    ...
    supervised.set_network(net_name)

    # If specified, load Deep SAD model (center c, network weights, and possibly autoencoder weights)
    if load_model:
        supervised.load_model(model_path=load_model,
                              load_ae=True,
                              map_location=device)
    ...
    supervised.save_results(export_json=xp_path + '/results.json')
    supervised.save_model(export_model=xp_path + '/model.tar')
    cfg.save_config(export_json=xp_path + '/config.json')

    # Plot most anomalous and most normal test samples
    indices, labels, scores = zip(*supervised.results['test_scores'])
