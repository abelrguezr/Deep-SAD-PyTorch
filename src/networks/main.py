...
def build_network(net_name, ae_net=None):
    ...
    if net_name == 'mnist_DGM_M2':
        net = DeepGenerativeModel([1 * 28 * 28, 2, 32, [128, 64]],
                                  classifier_net=MNIST_LeNet)

    if net_name == 'mnist_DGM_M1M2':
        net = StackedDeepGenerativeModel([1 * 28 * 28, 2, 32, [128, 64]],
                                         features=ae_net)
    ...
    if net_name == 'fmnist_DGM_M2':
        net = DeepGenerativeModel([1 * 28 * 28, 2, 64, [256, 128]],
                                  classifier_net=FashionMNIST_LeNet)

    if net_name == 'fmnist_DGM_M1M2':
        net = StackedDeepGenerativeModel([1 * 28 * 28, 2, 64, [256, 128]],
                                         features=ae_net)
    ...
    if net_name == 'cifar10_DGM_M2':
        net = DeepGenerativeModel([3 * 32 * 32, 2, 128, [512, 256]],
                                  classifier_net=CIFAR10_LeNet)

    if net_name == 'cifar10_DGM_M1M2':
        net = StackedDeepGenerativeModel([3 * 32 * 32, 2, 128, [512, 256]],
                                         features=ae_net)
...
def build_autoencoder(net_name):
    ...
    implemented_networks = ('mnist_LeNet', 'mnist_DGM_M1M2', 'fmnist_LeNet',
    ...
    if net_name == 'arrhythmia_mlp':
        ae_net = MLP_Autoencoder(x_dim=274,
                                 h_dims=[128, 64],
                                 rep_dim=32,
                                 bias=False)

    if net_name == 'cardio_mlp':
        ae_net = MLP_Autoencoder(x_dim=21,
                                 h_dims=[32, 16],
                                 rep_dim=8,
                                 bias=False)
    ...
    if net_name == 'satellite_mlp':
        ae_net = MLP_Autoencoder(x_dim=36,
                                 h_dims=[32, 16],
                                 rep_dim=8,
                                 bias=False)
    ...
    if net_name == 'shuttle_mlp':
        ae_net = MLP_Autoencoder(x_dim=9,
                                 h_dims=[32, 16],
                                 rep_dim=8,
                                 bias=False)

    if net_name == 'thyroid_mlp':
        ae_net = MLP_Autoencoder(x_dim=6,
                                 h_dims=[32, 16],
                                 rep_dim=4,
                                 bias=False)
