...
@click.argument('dataset_name',
                type=click.Choice([
                    ...
                    'satellite', 'cicflow', 'satimage-2', 'shuttle', 'thyroid'
                ]))
...
@click.option(
    ...
    default=256,
    help=
    'Set the number of samples drawn to train each base estimator (default: 256).'
)
@click.option(
    '--contamination',
    type=float,
...
@click.option(
    ...
    default=False,
    help=
    'Train model on features extracted from an autoencoder. If True, load_ae must be specified.'
)
@click.option(
    '--load_ae',
    type=click.Path(exists=True),
